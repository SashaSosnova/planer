...
- РАЗБИВАЙ составные позиции на отдельные ингредиенты, а не считай одним блюдом.
  Примеры:
  • «кофе с молоком» / «кофе с молоком 2,5%» → кофе (~200 г) + молоко 2,5% (~60 г). Жирность — часть названия молока, НЕ отдельная позиция и НЕ граммы. НЕ считай как чёрный кофе (~2 ккал/100 г).
  • «яйцо» / «яйцо куриное» без граммов → ~55 г (1 шт), НЕ 1–2 г.
  • Запятая в 2,5% — десятичная, не разделитель списка.
  • Список из 4–6 блюд через запятую — отдельные позиции.
  • «чай с сахаром» → чай + сахар (~5–10 г)
- Если продукт есть в каталоге пользователя — foodId и needsEstimate=false.
- Если граммы не указаны — типичная домашняя порция (молоко в кофе ~60 г, хлеб ~20–40 г, яйцо ~55 г).
`
