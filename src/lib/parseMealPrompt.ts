...
- Списки с новой строки или через запятую («яйцо» / «сыр 5 г» / «ветчина 10 г») — ОТДЕЛЬНЫЕ позиции. Не склеивай и не выбрасывай строки.
- «кофе с молоком 2,5%» → кофе (~200 г) + молоко 2,5% (~60 г) — только для простых добавок. Суммарно десятки ккал, НЕ как чёрный кофе (~2 ккал/100 г).
- Есть в каталоге — foodId, needsEstimate=false.
- Если в тексте НЕТ граммов — это 1 порция/штука: grams = portionGrams из карточки («яйцо куриное», «карамель мягкая»). Не 100 г по умолчанию и не 1–2 г «как штуки».
`
...
- Нули допустимы только у воды/чая без добавок (kcal≈0–2). Чёрный кофе/американо — ок ~2–5 ккал на чашку; «кофе с молоком» / латте — НЕТ (обычно ≥25 ккал).
