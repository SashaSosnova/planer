      ...
      <ul className="achievements-list">
        {items.map((a) => (
          <li key={a.id} className={`achievement-card${a.unlocked ? ' unlocked' : ''}`}>
            <span className="achievement-mark sticker-slot" aria-hidden>
              {a.unlocked ? a.sticker.character.slice(0, 1) : '?'}
            </span>
            <div>
  ...
  return (
    <section className="screen">
      <header className="screen-header">
        ...
        <h1>Достижения</h1>
        <p className="muted">
          {unlocked} из {total} · за привычки и заботу о себе, не за идеальные цифры
        </p>
      </header>
